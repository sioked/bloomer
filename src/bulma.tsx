...
import * as CoreGrid from './grid/grid';
import {
  combineModifiers,
  getHTMLProps,
  is,
...
export interface Grid extends CoreGrid.HorizontalSize, CoreGrid.Offset {}
...
export function getAlignmentModifiers({isAlign: align}: Alignment) {
  return isAlign(align) ? {[`is-${align}`]: true} : {};
}

export function removeAlignmentProps(props: Alignment) {
  const {isAlign, ...rest} = props;
  return rest;
}

export function getSizeModifiers({isSize: size}: Size) {
  return isSize(size) ? {[`is-${size}`]: true} : {};
}

export function removeSizeProps(props: Size) {
  const {isSize, ...rest} = props;
  return rest;
}

function getFullWidthModifiers(props: FullWidth) {
  return {
    'is-fullwidth': props.isFullWidth,
  };
}

function removeFullWidthProps(props: FullWidth) {
  const {isFullWidth, ...rest} = props;
  return rest;
}

export function getActiveModifiers(props: Active) {
  return {'is-active': props.isActive};
}

export function removeActiveModifiers(props: Active) {
  const {isActive, ...rest} = props;
  return rest;
}

export function getFocusedModifiers(props: Focused) {
  return {'is-focused': props.isFocused};
}

export function removeFocusedModifiers(props: Focused) {
  const {isFocused, ...rest} = props;
  return rest;
}

export function getHoveredModifiers(props: Hovered) {
  return {'is-hovered': props.isHovered};
}

export function removeHoveredModifiers(props: Hovered) {
  const {isHovered, ...rest} = props;
  return rest;
}

export function getStateModifiers(props: State) {
  return {
...
export function removeStateProps(props: State) {
  const {isActive, isFocused, isHovered, ...rest} = props;
  return rest;
}

export function getLoadingModifiers(props: Loading) {
  return {
    'is-loading': props.isLoading,
  };
}

export function removeLoadingProps(props: Loading) {
  const {isLoading, ...rest} = props;
  return rest;
}

export function getColorModifiers({isColor: color}: Color) {
  return color ? {[`is-${color}`]: true} : {};
}

export function removeColorProps(props: Color) {
...
export function getHeadingModifiers({isSpaced, isSize: size}: Heading) {
...
export function removeHeadingProps(props: Heading) {
...
const getShowModifiers = (display: Displays | Displays[] | DisplayObject) => {
...
const getHideModifiers = (
  platform: boolean | AllPlatforms | AllPlatforms[],
) => {
...
function removeHelpersProps(props: Helpers) {
  const {
    isDisplay,
    isHidden,
    isClearfix,
...
export function withHelpersModifiers<T>(Component: Component<T>) {
  const SFC: React.SFC<T & React.HTMLProps<HTMLElement> & Helpers> = ( props ) => {
